...
export type Struct_598 = {
  unk0: number;
  bossKillDataList: BossKillData.BossKillData[];
  struct_1: { unk1_0_0: number; struct_514: Buffer }[];
  unk3: number;
  unk4: number;
};
export function read(reader: Read) {
  const data = {} as Struct_598;
  data.unk0 = reader.u32();
  data.bossKillDataList = reader.array(reader.u16(), () => BossKillData.read(reader), 3);
  data.struct_1 = reader.array(
    reader.u16(),
    () => {
      const p = {} as { unk1_0_0: number; struct_514: Buffer };
      p.unk1_0_0 = reader.u32();
      p.struct_514 = reader.bytes(reader.u16(), 10);
      return p;
    },
    3
  );
