...
export function write(writer: Write, data: InitItem | PKTInitItem) {
  writer.array(
    data.storageType === itemstoragetype.equip ? data.itemDataList : [], // We only log StorageType.Equip because we don't want to fill log files with full player inventory & stash
